...
#ifndef CruxCoin_ZCUXWALLET_H
#define CruxCoin_ZCUXWALLET_H
...
public:
    CzCUXWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZCUX(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZCUX(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //CruxCoin_ZCUXWALLET_H
