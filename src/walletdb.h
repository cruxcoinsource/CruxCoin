...
public:
    ...
    bool WriteZCUXSeed(const uint256& hashSeed, const vector<unsigned char>& seed);
    bool ReadZCUXSeed(const uint256& hashSeed, vector<unsigned char>& seed);
    bool ReadZCUXSeed_deprecated(uint256& seed);
    bool EraseZCUXSeed();
    bool EraseZCUXSeed_deprecated();

    bool WriteZCUXCount(const uint32_t& nCount);
    bool ReadZCUXCount(uint32_t& nCount);
