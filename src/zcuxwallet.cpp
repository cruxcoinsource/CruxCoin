...
CzCUXWallet::CzCUXWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zcux seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZCUXSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZCUXSeed_deprecated()) {
                    LogPrintf("%s: Updated zCUX seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zcux seed\n", __func__);
                }
            }
        }
    }
    ...
    if (fFirstRun) {
        ...
        seedMaster = seed;
        LogPrintf("%s: first run of zcux wallet detected, new seed generated. Seedhash=%s\n", __func__, Hash(seed.begin(), seed.end()).GetHex());
...
bool CzCUXWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZCUXCount(nCountLastUsed);
    else if (!walletdb.ReadZCUXCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzCUXWallet::Lock()
{
    seedMaster = 0;
}

void CzCUXWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzCUXWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZCUX(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzCUXWallet::LoadMintPoolFromDB()
{
...
void CzCUXWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzCUXWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzCUXWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zcuxTracker->HasPubcoinHash(pMint.first)) {
...
bool CzCUXWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZCUX(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zcuxTracker which also adds to database
    pwalletMain->zcuxTracker->Add(dMint, true);
...
void CzCUXWallet::SeedToZCUX(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzCUXWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzCUXWallet::GenerateDeterministicZCUX(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzCUXWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzCUXWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
