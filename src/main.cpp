...
#if defined(NDEBUG)
#error "CruxCoin cannot be compiled without assertions."
#endif
...
{
	//Check to see if the zCUX is properly signed
	if (pindex->nHeight >= Params().Zerocoin_Block_V2_Start()) {
		if (!spend.HasValidSignature())
			return error("%s: V2 zCUX spend does not have a valid signature", __func__);

		libzerocoin::SpendType expectedType = libzerocoin::SpendType::SPEND;
		if (tx.IsCoinStake())
			expectedType = libzerocoin::SpendType::STAKE;
		if (spend.getSpendType() != expectedType) {
			return error("%s: trying to spend zCUX without the correct spend type. txid=%s", __func__,
				tx.GetHash().GetHex());
		}
	}
	...
	if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
		return error("%s : zCUX spend with serial %s is already in block %d\n", __func__,
	...
	if (pindex->nHeight > Params().Zerocoin_Block_EnforceSerialRange() &&
		...
		return error("%s : zCUX spend with serial %s from tx %s is not in valid range\n", __func__,
...

	{
		...
		if (tx.IsZerocoinSpend()) {
			...
			if (IsTransactionInChain(tx.GetHash(), nHeightTx))
				return state.Invalid(error("AcceptToMemoryPool : zCUX spend tx %s already in block %d",
			...
			for (const CTxIn& txIn : tx.vin) {
				...
				if (!ContextualCheckZerocoinSpend(tx, spend, chainActive.Tip(), 0))
					return state.Invalid(error("%s: ContextualCheckZerocoinSpend failed for tx %s", __func__,
						tx.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zcux");
			}
		}
		else {
			LOCK(pool.cs);
			...
			// Check that zCUX mints are not already known
...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 26000 && nHeight > 22000) {
		nSubsidy = 6 * COIN;
	}
	else if (nHeight <= 30000 && nHeight > 26000) {
		nSubsidy = 6.6 * COIN;
	}
	else if (nHeight <= 34000 && nHeight > 30000) {
		nSubsidy = 7.2 * COIN;
	}
	else if (nHeight <= 40000 && nHeight > 34000) {
		nSubsidy = 7.8 * COIN;
	}
	else if (nHeight <= 44000 && nHeight > 40000) {
		nSubsidy = 8.4 * COIN;
	}
	else if (nHeight <= 50000 && nHeight > 44000) {
		nSubsidy = 9 * COIN;
	}
	else if (nHeight <= 54000 && nHeight > 50000) {
		nSubsidy = 9.6 * COIN;
	}
	else if (nHeight <= 58000 && nHeight > 54000) {
		nSubsidy = 10.2 * COIN;
	}
	else if (nHeight <= 62000 && nHeight > 58000) {
		nSubsidy = 10.8 * COIN;
	}
	else if (nHeight <= 66000 && nHeight > 62000) {
		nSubsidy = 11.4 * COIN;
	}
	else if (nHeight <= 70000 && nHeight > 66000) {
		nSubsidy = 12 * COIN;
	}
	else if (nHeight <= 74000 && nHeight > 70000) {
		nSubsidy = 12.6 * COIN;
	}
	else if (nHeight <= 78000 && nHeight > 74000) {
		nSubsidy = 13.2 * COIN;
	}
	else if (nHeight <= 82000 && nHeight > 78000) {
		nSubsidy = 13.8 * COIN;
	}
	else if (nHeight <= 84000 && nHeight > 82000) {
		nSubsidy = 14.4 * COIN;
	}
	else if (nHeight <= 88000 && nHeight > 84000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight <= 92000 && nHeight > 88000) {
		nSubsidy = 15.6 * COIN;
	}
	else if (nHeight <= 100000 && nHeight > 92000) {
		nSubsidy = 16.2 * COIN;
	}
	...
	else if (nHeight <= 260000 && nHeight > 230000) {
		nSubsidy = 24.6 * COIN;
	}
	else if (nHeight <= 290000 && nHeight > 260000) {
		nSubsidy = 24 * COIN;
	}
	else if (nHeight <= 320000 && nHeight > 290000) {
		nSubsidy = 23.4 * COIN;
	}
	...
	else if (nHeight <= 2100000 && nHeight > 520000) {
		nSubsidy = 15 * COIN;
	}
	else if (nHeight <= 4000000 && nHeight > 2100000) {
		nSubsidy = 10 * COIN;
	}
	else {
		nSubsidy = 1 * COIN;
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount, bool isZCUXStake)
{
	int64_t ret = 0;
	...
	else if (nHeight <= 18000 && nHeight > Params().LAST_POW_BLOCK()) {
		ret = blockValue * 4 / 5;  //80%;
	}
	else if (nHeight <= 22000 && nHeight > 18000) {
		ret = blockValue * 401 / 500;  //80.2%;
	}
	else if (nHeight <= 26000 && nHeight > 22000) {
		ret = blockValue * 201 / 250;  //80.4%;
	}
	else if (nHeight <= 30000 && nHeight > 26000) {
		ret = blockValue * 403 / 500;  //80.6%;
	}
	else if (nHeight <= 34000 && nHeight > 30000) {
		ret = blockValue * 101 / 125;  //80.8%;
	}
	else if (nHeight <= 40000 && nHeight > 34000) {
		ret = blockValue * 81 / 100;  //81%;
	}
	else if (nHeight <= 44000 && nHeight > 40000) {
		ret = blockValue * 203 / 250;  //81.2%;
	}
	else if (nHeight <= 50000 && nHeight > 44000) {
		ret = blockValue * 407 / 500;  //81.4%;
	}
	else if (nHeight <= 54000 && nHeight > 50000) {
		ret = blockValue * 102 / 125;  //81.6%;
	}
	else if (nHeight <= 58000 && nHeight > 54000) {
		ret = blockValue * 409 / 500;  //81.8%;
	}
	else if (nHeight <= 62000 && nHeight > 58000) {
		ret = blockValue * 41 / 50;  //82%;
	}
	else if (nHeight <= 66000 && nHeight > 62000) {
		ret = blockValue * 411 / 500;  //82.2%;
	}
	else if (nHeight <= 70000 && nHeight > 66000) {
		ret = blockValue * 103 / 125;  //82.4%;
	}
	else if (nHeight <= 74000 && nHeight > 70000) {
		ret = blockValue * 413 / 500;  //82.6%;
	}
	else if (nHeight <= 78000 && nHeight > 74000) {
		ret = blockValue * 207 / 250;  //82.8%;
	}
	else if (nHeight <= 82000 && nHeight > 78000) {
		ret = blockValue * 83 / 100;  //83%;
	}
	else if (nHeight <= 84000 && nHeight > 82000) {
		ret = blockValue * 104 / 125;  //83.2%;
	}
	else if (nHeight <= 88000 && nHeight > 84000) {
		ret = blockValue * 417 / 500;  //83.4%;
	}
	else if (nHeight <= 92000 && nHeight > 88000) {
		ret = blockValue * 209 / 250;  //83.6%;
	}
	else if (nHeight <= 100000 && nHeight > 92000) {
		ret = blockValue * 419 / 500;  //83.8%;
	}
	else if (nHeight <= 105000 && nHeight > 100000) {
		ret = blockValue * 21 / 25;  //84%;
	}
	else if (nHeight <= 110000 && nHeight > 105000) {
		ret = blockValue * 421 / 500;  //84.2%;
	}
	else if (nHeight <= 115000 && nHeight > 110000) {
		ret = blockValue * 211 / 250;  //84.4%;
	}
	else if (nHeight <= 120000 && nHeight > 115000) {
		ret = blockValue * 423 / 500;  //84.6%;
	}
	else if (nHeight <= 125000 && nHeight > 120000) {
		ret = blockValue * 106 / 125;  //84.8%;
	}
	else if (nHeight <= 130000 && nHeight > 125000) {
		ret = blockValue * 17 / 20;  //85%;
	}
	else if (nHeight <= 135000 && nHeight > 130000) {
		ret = blockValue * 213 / 250;  //85.2%;
	}
	else if (nHeight <= 140000 && nHeight > 135000) {
		ret = blockValue * 427 / 500;  //85.4%;
	}
	else if (nHeight <= 145000 && nHeight > 140000) {
		ret = blockValue * 107 / 125;  //85.6%;
	}
	else if (nHeight <= 150000 && nHeight > 145000) {
		ret = blockValue * 429 / 500;  //85.8%;
	}
	else if (nHeight <= 155000 && nHeight > 150000) {
		ret = blockValue * 43 / 50;  //86%;
	}
	else if (nHeight <= 160000 && nHeight > 155000) {
		ret = blockValue * 431 / 500;  //86.2%;
	}
	else if (nHeight <= 165000 && nHeight > 160000) {
		ret = blockValue * 108 / 125;  //86.4%;
	}
	else if (nHeight <= 170000 && nHeight > 165000) {
		ret = blockValue * 433 / 500;  //86.6%;
	}
	else if (nHeight <= 175000 && nHeight > 170000) {
		ret = blockValue * 217 / 250;  //86.8%;
	}
	else if (nHeight <= 180000 && nHeight > 175000) {
		ret = blockValue * 87 / 100;  //87%;
	}
	else if (nHeight <= 185000 && nHeight > 180000) {
		ret = blockValue * 109 / 125;  //87.2%;
	}
	else if (nHeight <= 190000 && nHeight > 185000) {
		ret = blockValue * 437 / 500;  //87.4%;
	}
	else if (nHeight <= 195000 && nHeight > 190000) {
		ret = blockValue * 219 / 250;  //87.6%;
	}
	else if (nHeight <= 200000 && nHeight > 195000) {
		ret = blockValue * 439 / 500;  //87.8%;
	}
	else if (nHeight <= 230000 && nHeight > 200000) {
		ret = blockValue * 22 / 25;  //88%;
	}
	else if (nHeight <= 260000 && nHeight > 230000) {
		ret = blockValue * 441 / 500;  //88.2%;
	}
	else if (nHeight <= 290000 && nHeight > 260000) {
		ret = blockValue * 221 / 250;  //88.4%;
	}
	else if (nHeight <= 320000 && nHeight > 290000) {
		ret = blockValue * 443 / 500;  //88.6%;
	}
	else if (nHeight <= 350000 && nHeight > 320000) {
		ret = blockValue * 111 / 125;  //88.8%;
	}
	else if (nHeight <= 360000 && nHeight > 350000) {
		ret = blockValue * 89 / 100;  //89%;
	}
	else if (nHeight <= 370000 && nHeight > 360000) {
		ret = blockValue * 223 / 250;  //89.2%;
	}
	else if (nHeight <= 380000 && nHeight > 370000) {
		ret = blockValue * 447 / 500;  //89.4%;
	}
	else if (nHeight <= 390000 && nHeight > 380000) {
		ret = blockValue * 112 / 125;  //89.6%;
	}
	else if (nHeight <= 400000 && nHeight > 390000) {
		ret = blockValue * 449 / 500;  //89.8%;
	}
	else {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
	...
	for (int i = block.vtx.size() - 1; i >= 0; i--) {
		...
		* note we only undo zerocoin databasing in the following statement, value to and from CruxCoin
...
{
	RenameThread("cruxcoin-scriptch");
	scriptcheckqueue.Thread();
}

void RecalculateZCUXMinted()
{
...
void RecalculateZCUXSpent()
{
	...
	while (true) {
		...
		//Remove spends from zCUX supply
...
bool RecalculateCUXSupply(int nHeightStart)
{
...
{
	// CruxCoin: recalculate Accumulator Checkpoints that failed to database properly
...
bool UpdateZCUXSupply(const CBlock& block, CBlockIndex* pindex)
{
	std::list<CZerocoinMint> listMints;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	if (pindex->nHeight == Params().Zerocoin_Block_RecalculateAccumulators() + 1) {
		RecalculateZCUXMinted();
		RecalculateZCUXSpent();
		RecalculateCUXSupply(Params().Zerocoin_StartHeight());
	}

	//Track zCUX money supply in the block index
	if (!UpdateZCUXSupply(block, pindex))
		return state.DoS(100, error("%s: Failed to calculate new zCUX supply for block=%s height=%d", __func__,
...
void static UpdateTip(CBlockIndex* pindexNew)
{
	...
	// If turned on AutoZeromint will automatically convert CUX to zCUX
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
	...
	for (const CTransaction& tx : block.vtx) {
		...
		// double check that there are no double spent zCUX spends in this block
		if (tx.IsZerocoinSpend()) {
			for (const CTxIn& txIn : tx.vin) {
				if (txIn.scriptSig.IsZerocoinSpend()) {
					libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
					if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
						return state.DoS(100, error("%s : Double spending of zCUX serial %s in block\n Block: %s",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	if (nHeight < Params().Zerocoin_Block_V2_Start())
		return error("%s: zCUX stake block is less than allowed start height", __func__);

	if (CZCuxStake* zCUX = dynamic_cast<CZCuxStake*>(stake)) {
		CBlockIndex* pindexFrom = zCUX->GetIndexFrom();
		if (!pindexFrom)
			return error("%s: failed to get index associated with zCUX stake checksum", __func__);

		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zCUX stake does not have required confirmation depth", __func__);
		...
		uint32_t nChecksum200 = ParseChecksum(nCheckpoint200, libzerocoin::AmountToZerocoinDenomination(zCUX->GetValue()));
		if (nChecksum200 != zCUX->GetChecksum())
			return error("%s: accumulator checksum is different than the block 200 blocks previous. stake=%d block200=%d", __func__, zCUX->GetChecksum(), nChecksum200);
	}
	else {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (block.IsProofOfStake()) {
		...
		if (stake->IsZCUX() && !ContextualCheckZerocoinStake(pindexPrev->nHeight, stake.get()))
			return state.DoS(100, error("%s: staked zCUX fails context checks", __func__));
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
	...
	if (nMints || nSpends)
		LogPrintf("%s : block contains %d zCUX mints and %d zCUX spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
	...
	if (strCommand == "version") {
		...
		// CruxCoin: We use certain sporks during IBD, so check to see if they are
