...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nDisplayUnit"))
        settings.setValue("nDisplayUnit", BitcoinUnits::CUX);
    ...
    if (!settings.contains("nAnonymizeCruxCoinAmount"))
        settings.setValue("nAnonymizeCruxCoinAmount", 1000);

    nAnonymizeCruxCoinAmount = settings.value("nAnonymizeCruxCoinAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeCruxCoinAmount"))
        SoftSetArg("-anonymizecruxcoinamount", settings.value("nAnonymizeCruxCoinAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in cruxcoin.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCruxCoinAmount:
            return QVariant(nAnonymizeCruxCoinAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeCruxCoinAmount:
            nAnonymizeCruxCoinAmount = value.toInt();
            settings.setValue("nAnonymizeCruxCoinAmount", nAnonymizeCruxCoinAmount);
            emit anonymizeCruxCoinAmountChanged(nAnonymizeCruxCoinAmount);
            break;
        case CoinControlFeatures:
