...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction& transaction, const CCoinControl* coinControl)
{
    ...
    foreach (const SendCoinsRecipient& rcp, recipients) {
        if (rcp.paymentRequest.IsInitialized()) { // PaymentRequest...
            ...
            total += subtotal;
        } else { // User-entered cruxcoin address / amount:
...

    {
        ...
        if (recipients[0].useSwiftTX && total > GetSporkValue(SPORK_5_MAX_VALUE) * COIN) {
            emit message(tr("Send Coins"), tr("SwiftX doesn't support sending values that high yet. Transactions are currently limited to %1 CUX.").arg(GetSporkValue(SPORK_5_MAX_VALUE)),
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction& transaction)
{
...

    {
        ...
        foreach (const SendCoinsRecipient& rcp, recipients) {
            ...
            } else if (!rcp.message.isEmpty()) // Message from normal cruxcoin:URI (cruxcoin:XyZ...?message=example)
            {
...
static void NotifyzCUXReset(WalletModel* walletmodel)
{
    qDebug() << "NotifyzCUXReset";
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyzCUXReset.connect(boost::bind(NotifyzCUXReset, this));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyzCUXReset.disconnect(boost::bind(NotifyzCUXReset, this));
...
{
    setMints.clear();
    setMints = pwalletMain->zcuxTracker->ListMints(fUnusedOnly, fMaturedOnly, fUpdateStatus);
}
